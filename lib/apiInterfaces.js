...
function jsonHttpRequest2 (host, port, data, callback, path) {
    path = path || '/json_rpc';
    callback = callback || function () {};
    var options = {
        hostname: host,
    ...
    var req = (port === 443 ? https : http)
        .request(options, function (res) {
            ...
            res.on('end', function () {
                // console.log("Raw response data:", replyData); 
                
                var replyJson;
                try {
                    
                    let cleanedReplyData = replyData.trim();
                    
                    if (cleanedReplyData.startsWith('{') || cleanedReplyData.startsWith('[')) {
                        replyJson = JSON.parse(cleanedReplyData);
                    } else {
                        throw new Error("Response is not valid JSON");
                    }
                } catch (e) {
                    // console.error("Error parsing JSON:", e.message);
                    // console.log("Invalid response content:", replyData);
                    callback(e, {}); 
                    return;
                }
                callback(null, replyJson); 
...
module.exports = function (daemonConfig, walletConfig, poolApiConfig) {
    return {
        ...
        jsonHttpRequest2: jsonHttpRequest2
    }
};
